#!/usr/bin/env python3
"""
domain_summary.py - Integrate BLAST and HHSearch results for domain analysis
Adapted from single_domain_summary.pl
"""
...
class DomainSummary:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.logger = logging.getLogger("ecod.domain_summary")
    ...
    def create_summary(self, pdb_id: str, chain_id: str, reference: str, 
        ...
        chain_blast_path = os.path.join(job_dump_dir, pdb_chain, 
                                      f"{pdb_chain}.{reference}.chainwise_blast.xml")
        if not os.path.exists(chain_blast_path):
        ...
        blast_path = os.path.join(job_dump_dir, pdb_chain, f"{pdb_chain}.{reference}.blast.xml")
...
def main():
    parser = argparse.ArgumentParser(description='Domain Summary Generator')
    ...
    summary = DomainSummary(args.config)
    
    # Set force overwrite option if provided
    if args.force_overwrite:
        summary.config['force_overwrite'] = True
    
    # Create summary
    output_file = summary.create_summary(
        args.pdb_id, 
        args.chain_id, 
        args.reference, 
        args.dump_dir,
    ...
    if output_file:
        print(f"Domain summary created: {output_file}")
    else:
        print("Failed to create domain summary")
