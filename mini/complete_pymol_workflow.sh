...
if [ ! -f "$REFERENCE_FILE" ]; then
    ...
    python mini/range_cache_parser.py --cache-file "$CACHE_FILE" --output-dir test_data_range_cache
...
if [ ! -f "$NEW_DOMAINS" ]; then
    ...
    python mini/scripts/quick_test.py "$PROTEIN_ID" \
        --batch-dir "$BATCH_DIR" \
...
python mini/scripts/run_pymol_comparison.py "$PROTEIN_ID" \
    --batch-dir "$BATCH_DIR" \
    --output-dir "/tmp"
