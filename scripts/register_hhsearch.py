...
class HHRegistrar:
    """Register and convert HHSearch results"""
    ...
    def process_batch(self, batch_id: int, force: bool = False, specific_chains: List[str] = None) -> Tuple[int, int]:
    ...
    def _find_chains_with_hhr(self, batch_id: int, batch_info: Dict[str, Any], 
        ...
        if specific_chains:
            ...
            # Get all chains in the batch with completed HHSearch
            query = """
            SELECT 
                ps.id as process_id,
                p.id as protein_id,
            ...
            WHERE 
                ...
                AND ps.current_stage = 'hhsearch_complete'
            """
            
            registered_rows = self.db.execute_dict_query(query, (batch_id,))
            self.logger.info(f"Found {len(registered_rows)} chains with completed HHSearch in database")
            
            # Also find chains with HHSearch results but not properly registered
            query = """
            SELECT 
                ps.id as process_id,
                p.id as protein_id,
            ...
            unregistered_rows = self.db.execute_dict_query(query, (batch_id,))
            self.logger.info(f"Found {len(unregistered_rows)} chains that might have HHSearch results but aren't registered")
            
            # Process registered chains first
            for chain in registered_rows:
                hhr_path = self._find_hhr_file(chain, batch_info)
                if hhr_path:
                    chain['hhr_path'] = hhr_path
                    chains.append(chain)
            
            # Then process potentially unregistered chains
            for chain in unregistered_rows:
                hhr_path = self._find_hhr_file(chain, batch_info)
                if hhr_path:
                    chain['hhr_path'] = hhr_path
                    chains.append(chain)
        
        self.logger.info(f"Found a total of {len(chains)} chains with HHR files")
        return chains
    
    def _find_hhr_file(self, chain: Dict[str, Any], batch_info: Dict[str, Any]) -> Optional[str]:
    ...
    def _register_hhr(self, chain: Dict[str, Any], batch_info: Dict[str, Any], force: bool = False) -> bool:
    ...
    def _convert_hhr_to_xml(self, chain: Dict[str, Any], batch_info: Dict[str, Any], force: bool = False) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to save XML file: {xml_path}, error: {e}")
            return False
        
        # Register XML
        return self._register_xml(process_id, xml_path, base_path)
    
    def _register_xml(self, process_id: int, xml_path: str, base_path: str) -> bool:
