...
class StatusTracker:
    ...
    def update_non_representative_status(self, batch_id: int) -> bool:
        """
        Update status for non-representative proteins when processing representatives only.
        ...
        try:
            ...
            if self.db_available:
                try:
                    # Mark non-representative proteins as skipped
                    query = """
                        UPDATE process_status
                        SET status = 'skipped', stage = 'skipped',
                            last_updated = %s, metadata = metadata || %s
                        WHERE batch_id = %s AND process_id NOT IN (
                            SELECT process_id FROM process_status
                            WHERE batch_id = %s AND metadata->>'is_representative' = 'true'
                        )
                    """

                    metadata = json.dumps({'skip_reason': 'non_representative', 'skipped_at': datetime.now().isoformat()})
                    params = (datetime.now(), metadata, batch_id, batch_id)

                    self.db_manager.execute_query(query, params)

                    self.logger.info(f"Updated non-representative protein status for batch {batch_id}")
                    return True

                except Exception as e:
                    self.logger.warning(f"Error updating non-representative status in database: {str(e)}")
                    # Continue with in-memory update

            # Update in-memory processes
            updated_count = 0
            for process_info in self.processes.values():
                if (process_info.batch_id == batch_id and
                    not process_info.metadata.get('is_representative', False) and
                    process_info.status == ProcessStatus.PENDING):

                    process_info.status = ProcessStatus.CANCELLED
                    process_info.stage = ProcessStage.SKIPPED
                    process_info.metadata['skip_reason'] = 'non_representative'
                    process_info.last_updated = datetime.now()
                    updated_count += 1

            self.logger.info(f"Updated {updated_count} non-representative proteins in memory for batch {batch_id}")
            return True

        except Exception as e:
            self.logger.error(f"Error updating non-representative status for batch {batch_id}: {str(e)}")
            return False

    @property
    def db(self):
        """Compatibility property for legacy code that expects 'db' attribute"""
