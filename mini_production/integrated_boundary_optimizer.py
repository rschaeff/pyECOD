...
class IntegratedBoundaryOptimizer:
    ...
    def dump_superposed_structures(self, structures: List[Protein3DStructure],
                                  output_dir: str = "superposed_structures") -> str:
        """Dump superposed structures as PDB files for PyMOL inspection"""

        import os
        os.makedirs(output_dir, exist_ok=True)

        print(f"💾 Dumping {len(structures)} superposed structures to {output_dir}/")

        pdb_files = []
        pymol_script_lines = [
            "# PyMOL script to load all superposed structures",
            "# Generated by integrated_boundary_optimizer.py",
            "",
            "delete all",
            "bg_color white",
            "",
        ...
        for i, structure in enumerate(structures):
            ...
            protein_id = structure.protein_result.protein_id
            pdb_file = os.path.join(output_dir, f"{protein_id}_superposed.pdb")
            pdb_files.append(pdb_file)

            # Write PDB file with aligned coordinates
            with open(pdb_file, 'w') as f:
                f.write(f"HEADER    SUPERPOSED STRUCTURE {protein_id}\n")
                f.write(f"REMARK    Original PDB: {structure.protein_result.pdb_id}\n")
                f.write(f"REMARK    Chain: {structure.protein_result.chain_id}\n")
                f.write(f"REMARK    Alignment RMSD: {structure.alignment_rmsd:.2f} Angstroms\n")
                f.write(f"REMARK    Boundary position: {structure.boundary_position_seq}\n")

                if structure.boundary_coordinates_3d is not None:
                    f.write(f"REMARK    Boundary coords: {structure.boundary_coordinates_3d[0]:.1f} {structure.boundary_coordinates_3d[1]:.1f} {structure.boundary_coordinates_3d[2]:.1f}\n")

                f.write("REMARK\n")

                # Write CA atoms with aligned coordinates
                for j, coords in enumerate(structure.aligned_coordinates):
                    res_num = j + 1  # 1-based residue numbering

                    # Mark boundary residue specially
                    if res_num == structure.boundary_position_seq:
                        atom_name = "CB"  # Use CB to highlight boundary
                        element = "C"
                    else:
                        atom_name = "CA"
                        element = "C"

                    line = f"ATOM  {j+1:5d}  {atom_name:<3} ALA A{res_num:4d}    {coords[0]:8.3f}{coords[1]:8.3f}{coords[2]:8.3f}  1.00 20.00           {element}\n"
                    f.write(line)

                f.write("END\n")

            # Add to PyMOL script
            object_name = f"{protein_id}"
            color = self.get_pymol_color(i)

            pymol_script_lines.extend([
                f"# Structure {i+1}: {protein_id} (RMSD: {structure.alignment_rmsd:.1f}Å)",
            ...
            # Highlight boundary residue
            if structure.boundary_position_seq:
                pymol_script_lines.extend([
                    f"# Highlight boundary residue for {protein_id}",
                    f"select {object_name}_boundary, {object_name} and resi {structure.boundary_position_seq}",
                    f"show spheres, {object_name}_boundary",
                    f"color red, {object_name}_boundary",
                    ""
                ])

        # Complete PyMOL script
        pymol_script_lines.extend([
            "# Final setup",
            "orient",
            "zoom all",
            ...
            "select all_boundaries, *_boundary",
            "show spheres, all_boundaries",
            "",
            "# Commands to try:",
            "# hide everything, all",
            "# show cartoon, all",
            "# show spheres, all_boundaries",
            "# orient all_boundaries  # Focus on boundary region",
            "",
            f"# {len(structures)} structures loaded",
            f"# Red spheres = domain boundaries",
            f"# Use 'select good_align' to pick well-aligned structures",
        ])

        # Write PyMOL script
        pymol_script_path = os.path.join(output_dir, "load_all_superposed.pml")
        with open(pymol_script_path, 'w') as f:
            f.write('\n'.join(pymol_script_lines))

        print(f"✓ Wrote {len(pdb_files)} PDB files to {output_dir}/")
        print(f"✓ Created PyMOL script: {pymol_script_path}")
        print(f"\n🎬 To view in PyMOL:")
        print(f"   pymol {pymol_script_path}")
        print(f"\n🔍 Analysis tips:")
        print(f"   • Red spheres = domain boundaries")
        print(f"   • Look for structures that cluster together")
        print(f"   • Check if boundaries align spatially")
        print(f"   • Hide/show individual structures to compare")

        return pymol_script_path

    def get_pymol_color(self, index: int) -> str:
        """Get PyMOL color for structure index"""
        colors = [
            "cyan", "yellow", "purple", "orange", "pink", "brown",
            "gray", "olive", "navy", "teal", "lime", "aqua", "silver",
            "maroon", "fuchsia", "green", "blue", "red"
        ]
