...
class ReferenceDomainRecovery:
    ...
    def query_reference_domain(self, classification: ClassificationLookup) -> Optional[ReferenceDomain]:
        ...
        with self.db_conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
            ...
            # Fixed to use actual schema: protein.source_id (not pdb_id)
            cursor.execute("""
                SELECT
                    d.ecod_domain_id,
                    d.ecod_uid,
                    p.source_id as pdb_id,    -- source_id contains PDB ID
                    p.chain_id,
                    eth.f_id as t_group,
                    eth.h_id as h_group,
                    eth.x_id as x_group,
                ...
                JOIN
                    public.protein p ON d.ecod_source_id = p.source_id
                                    AND d.ecod_chain_id = p.chain_id
                LEFT JOIN
                ...
                WHERE
                    ...
                    AND p.source = 'pdb'  -- Only PDB entries
                    AND eth.f_id = %s
            ...
            return ReferenceDomain(
                ...
                pdb_id=row['pdb_id'],  # This is actually source_id
    ...
    def print_summary(self, batch_results: List[Dict] = None):
        ...
        print(f"Database queries:     {self.stats['db_queries']:,}")
