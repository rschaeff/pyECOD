...
class CoordinateValidator:
    ...
    def _load_config(self, config_path: str = None) -> dict:
        """Load database configuration from specified config file"""
            ...
            try:
                with open(config_path, 'r') as f:
                    ...
                    if 'database' in config:
                        print(f"✅ Using database credentials from {config_path}")
                        return config
                    else:
                        print(f"⚠️ No database section found in {config_path}")
            ...
            if os.path.exists(config_file):
                try:
                    with open(config_file, 'r') as f:
                        config = yaml.safe_load(f)
                        if 'database' in config:
                            print(f"✅ Using database credentials from {config_file}")
                            return config
                except Exception as e:
                    continue

        print("⚠️ No database configuration found - running structure-only validation")
    ...
    def _connect_database(self):
        ...
        if not self.config.get('database'):
            print("⚠️ No database configuration - validation will be structure-only")
            return

        try:
            db_config = self.config['database']

            # Handle both 'database' and 'dbname' keys (your config uses 'database')
            dbname = db_config.get('database') or db_config.get('dbname', 'ecod_protein')
            ...
            print(f"✅ Connected to ECOD database at {db_config['host']}:{db_config.get('port', 5432)}")
        except Exception as e:
...
def main():
    ...
    parser.add_argument('--config', default='config/config.local.yml',
                        help='Database config file (default: config/config.local.yml)')
